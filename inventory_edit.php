...
// SUCCESS NOTE
	...
	}else if (isset($_GET['success_update']) && empty($_GET['success_update'])){
		echo "<div class='general-note' style='height:50px; padding-top: 40px;display:block; margin-bottom:9px; margin-top:-9px; margin-bottom: 0px;'>
				<p>Selected product successfully updated. Please check it &nbsp; <a href='inventory.php' style='text-decoration: underline; color:orange;'><u>out.</u></a></p>
			   </div>";	
	}else
	{
?>
...
//READ THE PRODUCT DATA
	/*if(isset($_GET['update_id'])){
		...
		echo "<a href='inventory.php?yesupdate_product=". $_GET['delete_product'] ."' style='color:red;padding-left:3px; text-decoration:underline'>Yes</a> | ";
	...
	if(isset($_GET['update_id'])){
		...
		$product_data = product_data($product_id, 'prd_id', 'prd_name', 'prd_price', 'prd_desc', 'cat_id', 'prd_imgLoc');
	}
?>

<?php
//UPDATING A PRODUCT
	if (isset($_POST['action']) && $_POST['action'] == 'Update this product'){

		if(isset($_FILES['pr_img']) === true){
			if(empty($_FILES['pr_img']['name']) === true){
				$file_path = $product_data['prd_imgLoc'];
			}else{
		...
		if(empty($errors) === true){
			...
			update_product($register_data, $_POST['thisID']);
			header('Location: inventory_edit.php?success_update');
			exit();
		}
	}
?>
...
<div class ='container' id='inventory'>
	...
	<div class='data-container'>
		...
		<img src="<?php echo $product_data['prd_imgLoc']; ?>" id='prev-img' alt='Product Photo Here'>	
		<form action='' method='POST' enctype='multipart/form-data'>
			<label>Product name : </label>
			<input type='text' name='pr_name' required value="<?php echo $product_data['prd_name']; ?>"><br>
			<label>Price : </label>
			<input type='text' name='pr_price' onkeypress="return isNumberKey(event)" required value="<?php echo $product_data['prd_price']; ?>"/><br>
			...
			<select name='c_id' >
				<?php
					...
					while ($row = mysql_fetch_array($result)){
						if($row['cat_id'] == $product_data['cat_id']){
			...
			<textarea name='pr_desc'><?php echo $product_data['prd_desc']; ?></textarea>
			<input type='hidden' name='thisID' value='<?php echo $product_data['prd_id']; ?>'>
			<input id='button' type='submit' value='Update this product' name='action'>
		</form>
	...
	<div id='list-menu'>
		<ul>
			<form action='' method='POST'>
				...
				<input style='margin-left: 80px;' type='text' name='search' placeholder='Search Keyword'>
...
<?php } 
	if (isset($_GET['success_add']) || isset($_GET['success_delete']) || isset($_GET['success_update'])){
